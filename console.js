class csl {
    ...
    open() {
        ...
        this.#opened = true;
        const wrapper = makeElement("div", "console-wrapper"), header = makeElement("div", "console-header"), body = makeElement("div", "console-body");
        const pos = memoryManager.getItem("console-pos");
        this.#position = {
        ...
        const drag = (ev) => {
            const sty = getComputedStyle(wrapper), offsetX = parseInt(sty.left, 10) - ev.clientX, offsetY = parseInt(sty.top, 10) - ev.clientY;
    ...
    #pushAndLog(message) {
        ...
        if (["warn", "warn_severe", "error", "fatal_error"].includes(message.type)) {
            const csl = document.querySelector("div#menu-container button#console");
            if (csl) {
    ...
    #generateHTML(data) {
        const entry = makeElement("div", "", `console-entry console-entry-${data.type}`), timestamp = makeElement("div", "", "console-entry-tstp"), main = makeElement("div", "", "console-entry-cont");
        const d = (() => {
        ...
        if (typeof data.content != "object" && data.content !== null) {
            ...
            const expand = makeElement("span", "", "console-detail-toggle"), c = data.content;
            let expanded = false;
            expand.textContent = "▶";
    ...
    generateWarningWidget() {
        ...
        if (this.content.some(c => ["warn", "warn_severe", "error", "fatal_error"].includes(c.type))) {
            const warns = this.content.filter(c => c.type == "warn").length, severeWarns = this.content.filter(c => c.type == "warn_severe").length, errors = this.content.filter(c => c.type == "error").length, fatalErrors = this.content.filter(c => c.type == "fatal_error").length;
            div.append(...[
