const perf = {
    ...
    showMeters(tpsMode, fpsMode, config) {
        ...
        tpsMode = ({ 0: 0, 1: 1, 2: 2 }[tpsMode] ?? 0);
        fpsMode = ({ 0: 0, 1: 1, 2: 2 }[fpsMode] ?? 0);
        if (!$("debug-div")) {
        ...
        function createGraph(div) {
            const g = makeElement("canvas", "perf-graph"), gx = g.getContext("2d");
        ...
        function setTimers(logging) {
            ...
            perf._timers.c ??= setInterval(() => {
                ...
                const crt = +(perf._data.ticks < perf.config.critical_tickrate), crf = +(perf._data.frames < perf.config.critical_framerate), tickrate = `<span style="background-color: #00F2">TPS: ${crt ? `<span class="critical">` : ""}${perf._data.ticks}${crt ? "</span>" : ""}`, aT = +average((void 0), ...perf._data.tps), sT = +stdDev((void 0), ...perf._data.tps), avgT = `AVG: ${Math.round(100 * aT) / 100} ± ${Math.round(100 * sT) / 100}</span>`, framerate = `<span style="background-color: #FF02">FPS: ${crf ? `<span class="critical">` : ""}${perf._data.frames}${crf ? "</span>" : ""}`, aF = +average((void 0), ...perf._data.fps), sF = +stdDev((void 0), ...perf._data.fps), avgF = `AVG: ${Math.round(100 * aF) / 100} ± ${Math.round(100 * sF) / 100}</span>`;
                ...
                if (perf.mode.tps == 2 || perf.mode.fps == 2) {
                    const g = $("perf-graph"), gx = g.getContext("2d"), max = Math.max(...perf._data.tps, ...perf._data.fps), lt = perf._data.tps.length, lf = perf._data.fps.length, grdt = perf.mode.tps == 2 ? gx.createLinearGradient(0, g.height - (g.height * 2 * (perf.config.critical_tickrate / (1.5 * (max || 1)))), 0, g.height - (g.height * (perf.config.critical_tickrate / (1.5 * (max || 1))))) : (void 0), grdf = perf.mode.fps == 2 ? gx.createLinearGradient(0, g.height - (g.height * 2 * (perf.config.critical_framerate / (1.5 * (max || 1)))), 0, g.height - (g.height * (perf.config.critical_framerate / (1.5 * (max || 1))))) : (void 0);
