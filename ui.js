...
function drawUI() {
    ...
    // Reloading
    {
        if (p.state.reloading) {
            ...
            ctx.lineWidth = 50;
            ctx.strokeStyle = "#FFF";
            ctx.arc(425, 425, 400, -Math.PI / 2, ((gamespace._currentUpdate - p.state.reloading) / i.proto[i.proto.altReload && !i.ammo ? "altReload" : "reload"].duration) * 2 * Math.PI - Math.PI / 2);
            ctx.stroke();
            ctx.beginPath();
            ctx.lineWidth = 30;
    ...
    // HP
    const lag = $("ui-hp-bar-lag"), bar = $("ui-hp-bar"), percent = Math.max(0, 100 * gamespace.player.health / gamespace.player.maxHealth);
