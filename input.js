const keyBindings = (() => {
    const invSlotCommon = () => {
        const p = gamespace.player, i = p.inventory, ip = i.activeItem.proto, f = gamespace._currentUpdate - p.state.lastFreeSwitch >= 1000;
        ...
        "slot0": {
            ...
            callback: (type) => {
                if (type.endsWith("down") && gamespace.player !== void 0) {
                    const p = gamespace.player, i = p.inventory;
                    invSlotCommon();
                    i.activeIndex = 0;
        ...
        "next_item": {
            ...
            callback: (type) => {
                if (type.endsWith("down") && gamespace.player !== void 0) {
                    const p = gamespace.player, i = p.inventory;
                    invSlotCommon();
                    i.activeIndex = (i.activeIndex + 1) % 2;
                    if (!i.activeItem.ammo) {
                        i.activeItem.reload(p);
                    }
                }
