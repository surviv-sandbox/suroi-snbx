...
class Projectile {
     ...
     */
    get position() { return { x: this.#position.x, y: this.#position.y, z: this.layer }; }
    set position(v) {
    ...
    trajectory, drawFunc, subLayer = 0) {
        if (new.target === Projectile)
            throw new TypeError("Cannot instantiate abstract class 'Projectile'.");
            ...
            if ("distance" in trajectory) {
                if ("lifetime" in trajectory) { // OPTION 4
                    ...
                    this.#end = srvsdbx_Geometry.Vector2D
                        .fromPolarToVec(trajectory.direction, trajectory.distance)
                        .plus(this.#start)
                        .toPoint2D();
                }
                else { // OPTION 5
                ...
                this.#end = srvsdbx_Geometry.Vector2D
                    .fromPolarToVec(trajectory.direction, trajectory.speed * trajectory.lifetime)
                    .plus(this.#start)
                    .toPoint2D();
                this.#lifetime = trajectory.lifetime;
            }
    ...
    update() {
        if (this.#destroyed)
            return;
        const deltaTime = gamespace.currentUpdate - this.#created, lifetime = this.#lifetime;
     ...
     */
    reflect(origin, direction, source) {
        if (Number.isNaN(direction))
            return { err: "Direction was NaN" };
        if (!this.#reflectionRecord.has(source.id)) {
            this.#reflectionRecord.set(source.id, gamespace.currentUpdate);
            const range = srvsdbx_Geometry.Vector2D.distBetweenPts(this.#start, this.#end), dt = gamespace.currentUpdate - this.#created;
    ...
    modifyRange(range) {
        if (Number.isNaN(range))
            return { err: "Range was NaN" };
        const oldRange = srvsdbx_Geometry.Vector2D.distBetweenPts(this.#start, this.#end), lifetime = this.#lifetime, traveled = oldRange * ((gamespace.currentUpdate - this.#created) / lifetime);
        if (traveled >= range)
...
class Bullet extends Projectile {
    ...
    */
     ...
     */
    constructor(position, falloff, end, speed, damage, drawFunction, properties, persistance, effectsOnHit, emitter) {
        super({
                    ...
                    if (notInSet || persistance.allowMultipleHitsPerTarget) {
                        if (effectsOnHit?.length) {
                            for (let i = 0, l = effectsOnHit.length; i < l; i++) {
                                const effect = gamespace.prototypes.statusEffects.get(effectsOnHit[i]), effectAlreadyApplied = [...target.statusEffects.values()].find(s => s.prototype == effect);
                                if (effectAlreadyApplied)
                                    effectAlreadyApplied.renew();
                                else
                                    target.statusEffects.add(new StatusEffect(effect, target));
                            }
                        }
                        if (this.#isExplosive && props.explodeOnContact)
                            this.#explode(srvsdbx_Geometry.Vector2D.fromPoint2D(this.position).toPoint2D(), effectsOnHit);
                        else
                            target.applyDamage(this.#damage, emitter.owner);
                        if (notInSet) {
                        ...
                        if (persistance.hitsBeforeDespawn <= ++this.#hitCount) {
                            ...
                            */
                            const snap = ev.intersectionPoints.sort((a, b) => srvsdbx_Geometry.Vector2D.squaredDistBetweenPts(this.start, a)
                                /*   */ - srvsdbx_Geometry.Vector2D.squaredDistBetweenPts(this.start, b))[0];
                            this.setPosition(snap);
                            this.update = this.destroy;
                        }
                        new Particle(gamespace.prototypes.particles.get("srvsdbx::blood_splat"), srvsdbx_Geometry.Vector2D.clone(this.position), srvsdbx_Math.randomAngle());
                    }
                }
            });
     ...
     */
    static drawFromFirearm(emitter) {
        const prototype = emitter.prototype, ammoInfo = gamespace.prototypes.ammoTypes.get(prototype.caliber), images = ammoInfo.projectileInfo.images, image = pickRandomInArray(images), dimensions = srvsdbx_AssetManagement.determineImageDimensions(image.asset, prototype.ballistics.tracer);
        return function (p5) {
            if (this.destroyed)
                return;
            const deltaTime = gamespace.currentUpdate - this.created, props = this.#properties;
            p5.push();
            ...
            if (trail) {
                ...
                p5.tint(trail.tint);
                trailLength && p5.image(trail.image.asset, 0, 0, trail.width, trailLength);
                gamespace.drawHitboxIfEnabled(p5, "DEFAULT", () => p5.rect(0, 0, trail.width, trailLength));
                p5.pop();
            }
            if (this.#isExplosive) {
            ...
            if (prototype.suppressed) {
                const minOpacity = ammoInfo.alpha.min, maxOpacity = ammoInfo.alpha.max, opacityRate = ammoInfo.alpha.rate, direction = Math.sign(1 - opacityRate);
                if ((minOpacity < this.alpha && direction != -1) ||
            ...
            p5.tint(color);
            const width = dimensions.width, finalLength = prototype.ballistics.tracer.forceMaximumLength
                ? dimensions.height
                : Math.min(dimensions.height, srvsdbx_Geometry.Vector2D.distBetweenPts(this.position, this.start)), parrOffset = props.spinVel ? 0 : finalLength / 2;
            p5.rectMode(p5.CORNER);
            finalLength && p5.image(image.asset, 0, parrOffset, width, finalLength);
            gamespace.drawHitboxIfEnabled(p5, "DEFAULT", () => p5.rect(-width / 2, parrOffset - finalLength / 2, width, finalLength));
            p5.rotate(this.trajectory - this.angle);
            gamespace.drawDebug(p5, "COLLIDABLE", () => {
                p5.rect(0, 0, 1, srvsdbx_Geometry.Vector2D.distBetweenPts(this.position, this.lastPosition));
            }, () => {
                p5.translate(0, 0);
     ...
     */
    static drawFromExplosion(emitter) {
        ...
        return function (p5) {
            ...
            const finalLength = Math.min(height, srvsdbx_Geometry.Vector2D.distBetweenPts(this.position, this.start)), offset = (this.#properties.spinVel ? -1 : 1) * finalLength / 2;
            // p5 has this weird bug where 0-height images are drawn at full height?
            finalLength && p5.image(emitter.tracer.image.asset, 0, offset, width, finalLength);
     ...
     */
    static makeShrapnel(pos, shrapnelData, emitter) {
        const trajectory = srvsdbx_Math.randomAngle(), start = srvsdbx_Geometry.Vector2D.clone(pos), data = shrapnelData;
        return new Bullet(start, data.falloff, srvsdbx_Geometry.Vector2D.fromPolarToVec(trajectory, extractValue(data.range)).plus(start), data.velocity, data.damage, Bullet.drawFromExplosion(data), {
            type: "bullet",
     ...
     */
    #explode(position, effectsOnHit) {
        if (this.#properties.type != "explosive" || this.#hasExploded)
            return;
        const properties = this.#properties;
        // Omit makes me lose all the IntelliSense, so the alternative is this suboptimal cast lol
        this.#hasExploded = true;
        new Explosion(gamespace.prototypes.explosions.get(properties.explosionType), {
            x: position.x,
            y: position.y,
            z: this.layer
        }, this.#emitter, effectsOnHit);
    }
    update() {
        super.update();
        if (this.destroyed)
     ...
     */
    reflect(origin, direction, source) {
        const range = srvsdbx_Geometry.Vector2D.distBetweenPts(this.start, this.end), traveled = srvsdbx_Geometry.Vector2D.distBetweenPts(this.start, this.position);
        const baseReflect = srvsdbx_ErrorHandling.handleResult(super.reflect(origin, direction, source), () => { }, srvsdbx_ErrorHandling.identity);
        if (baseReflect)
            return { err: baseReflect };
        this.#damage *= 0.7;
        this.#opacity *= 0.8;
