...
class Generic {
     ...
     */
    #velocityMap = new ReducibleMap((acc, cur) => acc.plus(cur), srvsdbx_Geometry.Vector3D.zeroVec());
    /**
     * A map to collect velocities acting upon this object; these are then compiled into one net velocity each physics tick
     */
     ...
     */
    #angularVelocityMap = new ReducibleMap((acc, cur) => acc + cur, 0);
    /**
     * A map to collect angular velocities acting upon this object; these are then compiled into one net angular velocity each physics tick
     */
     ...
     */
    get position() { return { x: this.#body.position.x, y: this.#body.position.y, z: this.layer }; }
    /**
     ...
     */
    #parent = srvsdbx_ErrorHandling.Nothing;
    /**
     * The `Generic` to which this object is locked via `follow`
     */
     ...
     */
    #followOffset = { x: 0, y: 0, z: 0, parr: 0, perp: 0 };
    /**
     * The offset at which this object will follow its parent, relative to the parent's position
     * - `x`: The offset to apply along the x-axis
     * - `y`: The offset to apply along the y-axis
     ...
     */
    constructor(body, drawFunction, position, fillColor, strokeColor, angle = 0) {
        if (new.target === Generic)
            throw new TypeError("Cannot instantiate abstract class 'Generic'.");
        this.#body = body;
        Matter.Body.setPosition(body, position);
        Matter.Body.setAngle(body, angle);
        if ("z" in position)
            this.layer = position.z;
    ...
    static drawPolygon(p5) {
        ...
        drawShape();
        gamespace.drawDebug(p5, this.collidable ? "COLLIDABLE" : "DEFAULT", () => drawShape("close"), void 0, this);
        p5.pop();
    }
    /**
     ...
     * @param p5 The gamespace's p5 instance
     */
    static drawCircle(p5) {
        const body = this.body, radius = body.circleRadius;
        p5.push();
        this.strokeColor ? p5.stroke(this.strokeColor) : p5.noStroke();
        this.fillColor ? p5.fill(this.fillColor) : p5.noFill();
        p5.circle(0, 0, 2 * radius);
        p5.pop();
        gamespace.drawDebug(p5, this.collidable ? "COLLIDABLE" : "DEFAULT", () => p5.circle(0, 0, 2 * radius), void 0, this);
    }
    /**
    ...
    update() {
        if (this.#destroyed || this.#parent)
            return;
        const body = this.#body, deltaTime = gamespace.currentUpdate - gamespace.lastUpdate, velocity = this.compileVelocities();
        this.setPosition({
...
const gamespace = new (createSingleton(class Gamespace {
     ...
     */
    #cameraPosition = srvsdbx_Geometry.Vector2D.zeroPt();
    /**
    ...
    */
     ...
     */
    constructor() {
        function getAndSetIfAbsent(map, key) {
            if (map.has(key))
                return map.get(key);
            const newMap = {
                decals: new Map,
        ...
        ;
        function determineLayerMapKey(originalKey, gameObject) {
            if (gameObject instanceof Particle || gameObject instanceof Projectile) {
                return `${originalKey}${(gameObject.subLayer >= 0 ? "Over" : "Under")}`;
            }
            else {
                return originalKey;
            }
        ...
        for (const key in this.#objects) {
            const map = this.#objects[key];
            map.addEventListener("set", (_, __, gameObject) => {
                ...
                destinationMap[determineLayerMapKey(key, gameObject)].set(gameObject.id, gameObject);
            });
            map.addEventListener("delete", (_, id, gameObject) => {
                if (!gameObject)
                    return;
                const layer = this.#layeredObjects.get(gameObject.position.z)?.[determineLayerMapKey(key, gameObject)];
                if (!layer)
                    return;
                layer?.delete?.(id);
     ...
     */
    drawDebug(p5, style, hitboxCallback, transition, object) {
     ...
     */
    drawHitboxIfEnabled(p5, style, callback) {
     ...
     * @param object The generic object whose velocities should be drawn
     */
    drawVelocityIfEnabled(p5, object) {
        if (this.#settings.drawVelocities) {
            p5.push();
            const body = "body" in object ? object.body : object, linearScale = object instanceof Particle ? object.drag : 1, rotationScale = object instanceof Particle ? object.rotDrag : 1, velocity = (() => {
            ...
            })().scale(linearScale), angularVelocity = (object instanceof Projectile ? object.angularVelocity : object.compileAngularVelocities()) * rotationScale, magnitude = velocity.length, vectorLength = -1000 * magnitude, vectorWidth = 5, angularVelocityRadius = 20, linearColor = "#0000FF", angularColor = "#FF0000";
            ...
            if (magnitude)
                p5.triangle(-2 * vectorWidth, vectorLength, 2 * vectorWidth, vectorLength, 0, vectorLength - angularVelocityRadius);
            p5.noFill();
            ...
            if (Math.abs(angularVelocity) > 2 * Math.PI) {
                ...
                p5.arc(0, 0, angularVelocityRadius, angularVelocityRadius, ...(() => {
    ...
    update() {
        ...
        const deltaTime = this.#currentUpdate - this.#lastUpdate, p5 = this.#p5;
        ...
        this.#objectPool.forEach(gameObject => {
            ...
            if (gameObject instanceof Player) {
                ...
                const cam = p5._elements[0]._curCamera, x = gameObject.body?.position?.x, y = gameObject.body?.position?.y, shake = (() => {
                ...
                })(), cameraX = x ?? cam.eyeX, cameraY = y ?? cam.eyeY;
                this.#cameraPosition = {
                    x: x,
                    y: y
                };
                p5.camera(cameraX + shake.x, cameraY + shake.y, gameObject.view, (x ?? cam.centerX) + shake.x, (y ?? cam.centerY) + shake.y, 0);
            }
        });
        // Dispatch collision events
        const size = gamespace.PLAYER_SIZE, filter = [...this.#objectPool.values()].filter((value => value instanceof Generic)), bodies = filter.map(v => v.body), colliders = filter
            .filter((g) => g instanceof PlayerLike)
            .map(player => [...player.inventory.items.values()].filter(i => i instanceof Melee &&
            i.prototype.isReflective &&
            ((i.owner.activeItem != i && i.prototype.holstered?.collider) ||
                (i.owner.activeItem == i && i.prototype.worldObject?.collider))).map(m => {
            const collider = m.getCollider(), reference = m.owner.activeItem == m ? m.prototype.worldObject : m.prototype.holstered;
            Matter.Body.translate(collider, srvsdbx_Geometry.Vector2D.zeroVec()
        ...
        this.#objectPool.forEach(obj => {
            if (obj instanceof Generic || obj instanceof Projectile) {
                if ("collidable" in obj && !obj.collidable)
                    return;
                ...
                collision = (isGeneric ?
                    ...
                    Matter.Query.ray(bodies, obj.lastPosition, obj.position))[0], 
                /**
                ...
                */
                collision
                    ? collision.body
                        ?? (collision.bodyA == obj.body
                            ? collision.bodyB == obj.body
                                ? void 0
                                : collision.bodyB
                            : collision.bodyA)
                    : void 0, 
                ...
                reflectCollision = isGeneric
                    ? void 0
                    : colliders
                        .map(collider => srvsdbx_Geometry.collisionFunctions.segmentPolygon({ start: obj.lastPosition, end: obj.position }, collider[1].vertices).map(p => [collider[0], collider[1], p[0], p[1]]))
                        .flat()
                        .sort(([, , ptA], [, , ptB]) => srvsdbx_Geometry.Vector2D.squaredDistBetweenPts(ptA, obj.start) -
                        srvsdbx_Geometry.Vector2D.squaredDistBetweenPts(ptB, obj.start))[0], 
                /**
                 * Stores the point of collision between this projectile and a player, if it exists
                 */
                bulletPlayerCollision = collision
                    && obj instanceof Projectile && target instanceof PlayerLike
                    ? [
                        (srvsdbx_Geometry.collisionFunctions.segmentCircle({ start: obj.lastPosition, end: obj.position }, { origin: target.position, radius: target.radius }) ?? []),
                        target
                    ]
                    : void 0;
                if (!(collidedBody || reflectCollision))
                    return;
                if (obj instanceof Projectile) {
                    ...
                    const parsedCollision = (() => {
                        if (reflectCollision && bulletPlayerCollision) {
                            ...
                            const [r, b] = [
                                ...
                                srvsdbx_Geometry.Vector2D.squaredDistBetweenPts(bulletPlayerCollision[0].sort((a, b) => srvsdbx_Geometry.Vector2D.squaredDistBetweenPts(a, obj.start) -
                                    srvsdbx_Geometry.Vector2D.squaredDistBetweenPts(b, obj.start))[0], obj.start)
                            ];
                    ...
                    if (parsedCollision == reflectCollision) {
                        ...
                        */
                        const equal = srvsdbx_Geometry.Vector2D.squaredDistBetweenPts(reflectCollision[2], obj.lastPosition) <= 1e-3 ** 2, a = equal ? srvsdbx_Geometry.Vector2D.minus(obj.lastPosition, obj.position) : obj.lastPosition, b = reflectCollision[2], ab = srvsdbx_Geometry.Vector2D.fromPoint2D(b).minus(a), cd = srvsdbx_Geometry.Vector2D.fromPoint2D(reflectCollision[3].end).minus(reflectCollision[3].start);
                        obj.reflect(reflectCollision[2], ab.projection(cd) // step 1
                            .scale(2, true) // step 2
                            .plus(a, true) // step 2
                            .minus(b, true) // step 3
                            .direction
                            // Reflections aren't perfect
                            + srvsdbx_Math.toRad(srvsdbx_Math.meanDevPM_random(0, 5, true), "degrees"), reflectCollision[0]);
                    }
                    else {
                        obj.events.dispatchEvent(new CollisionEvent(collision, bulletPlayerCollision[0]), bulletPlayerCollision[1]);
                    }
                }
            }
        });
        // Draw the ground
        p5.clear();
        const level = gamespace.#currentLevel, { width, height } = level.world;
        ...
        for (const [_, layer] of this.#layeredObjects) {
            ...
            for (let i = 0, l = objects.length; i < l; i++)
                for (const [_, object] of objects[i])
                    ...
                    srvsdbx_Geometry.Vector2D.squaredDistBetweenPts(this.#cameraPosition, object.position) < srvsdbx_Geometry.Vector2D.squaredDist({ x: window.innerWidth, y: window.innerHeight })
...
class ImportedObject {
    ...
    */
     ...
     */
    constructor(name, displayName, targetVersion, namespace, includePath) {
        this.#name = name;
